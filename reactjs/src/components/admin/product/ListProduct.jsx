...
import { getProduct, removeProduct } from '../../../apis/product'
...
const ListProduct = (props) => {
  ...
  const onHandleRemove = async (slug) => {
    if(window.confirm("Ban muon xoa khong?")){
      const { data } = await removeProduct(slug)
    ...
    <div>
      <div className="bg-white shadow">
          ...
          <div className='max-w-7xl mx-auto py-6 sm:px-6 lg:px-8'>
            <Link to="/admin/product/add"><button className='border rounded-md shadow-sm text-white  px-6 py-3 bg-blue-500 font-[18px] hover:bg-white hover:text-black transition-all duration-300 ease-linear'>Add new product</button></Link>
          </div>
          <div>
              <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col" class="text-center px-6 py-3 text-[14px] font-bold text-gray-500 uppercase tracking-wider">Category</th>
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        {product?.map((item, index) => 
                                          <tr key={item.name}>
                                              ...
                                              <td class="text-center py-[5px]">{item.category?.name}</td>
                                              <td class="text-center py-[5px]">{item.price}</td>
